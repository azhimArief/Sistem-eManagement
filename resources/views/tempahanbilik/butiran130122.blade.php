...
<div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6"card card-orange card-tabs">
            <h1 class="m-0">Butiran Maklumat Permohonan</h1>
          </div><!-- /.col -->
		  ...
		  @if (session('status'))
			...
			<div class="card-body">
				<dl class="row">
					...
					<dd class="col-sm-4">{{ ($permohonanKenderaan->keterangan_lanjut!='') ? $permohonanKenderaan->keterangan_lanjut :'-' }}</dd>
			...
			<div class="card-header">
				...
				<span style="margin-left: 20px">
					<a href="{{ url('tempahankenderaan/kemaskini_tindakan',$permohonanKenderaan->id_maklumat_permohonan) }}" title="Kemaskini" class="btn-md"><i class="fa fa-edit"></i></a>
				</span>
				@endif
			</div>
			<div class="card-body">
				<dl class="row">
					...
					<thead bgcolor ="#e2e7e7">
						<tr>
							...
							@else
							<th><mark style="background-color: #FFFF10">Kenderaan</mark></th>
							@endif
						</tr>
					</thead>
					<tbody>
						<tr>
						<td bgcolor = "#f0f1f1">
								<p><strong> No Kenderaan</strong><span style="margin-left: 195px;"></span>
								{{ ($tindakan->kenderaan_pergi!='') ? \App\Kenderaan::find($tindakan->kenderaan_pergi)->no_plat : 'N/A' }}</p>
								<p><strong>Jenis Kenderaan</strong><span style="margin-left: 178px;"></span>
								{{ ($tindakan->kenderaan_pergi!='') ? $kenderaanPergi->jenis_kenderaan : 'N/A' }}</p>	
								<p><strong>Model Kenderaan</strong><span style="margin-left: 172px;"></span>
								{{ ($tindakan->kenderaan_pergi!='') ? $kenderaanPergi->jenis_model : 'N/A' }}</p>		
								<p><strong>Nama Pemandu</strong><span style="margin-left: 183px;"></span>
								{{ ($tindakan->kenderaan_pergi!='') ? $kenderaanPergi->nama_pemandu : 'N/A' }}</p>
								<p><strong>No. Tel. Bimbit Pemandu</strong><span style="margin-left: 120px;"></span>
			...
			<div class="card-body table-responsive">
				...
				@foreach ($tindakans as $tindakan1)
                  <tr>
					...
					<td> {{ ($tindakan1->peg_penyelia!='') ? \App\User::find($tindakan1->peg_penyelia)->nama : '' }} </td>
			...
			@if (Auth::user()->id_access == '2')
				...
				@endif
			<!-- if user PENYELIA -->
			@elseif (Auth::user()->id_access == '1')
				@if($permohonanKenderaan->id_status == 1 || $permohonanKenderaan->id_status == 2 || $permohonanKenderaan->id_status == 5)
				<a href="{{ url('tempahankenderaan/kemaskini/'.$permohonanKenderaan->id_maklumat_permohonan) }}" class="btn btn-info">Kemaskini</a>
				@endif
				@if(!isset($tindakan))
				<a href="{{ url('tempahankenderaan/tindakan/'.$permohonanKenderaan->id_maklumat_permohonan) }}" class="btn btn-primary" >Tindakan</a>
				<!-- <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#modalTindakan">Tindakan</button> -->
				<button type="button" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#agihan"> <i class="fas fa-bell"></i> |Tindakan</button>
