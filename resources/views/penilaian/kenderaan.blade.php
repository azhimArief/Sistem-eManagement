        ...
        <div class="row">
		  ...
		  @if (session('status'))
            ...
            <div class="card card-info">
				...
				<!-- /.card-header -->
				<div class="card-body" style="margin:10 40 40 40;">
				<form class="form-horizontal" method="POST" action="{{ url('penilaian/kenderaan/'.$permohonanKenderaan->id_maklumat_permohonan) }}" enctype="multipart/form-data">
				{{ csrf_field() }}
					...
					<div class="form-group row">
						...
						<div class="col-sm-12" style="margin-right:60px;">
							<input type="radio" id="skala[{{  $lkpSoalan->id_soalan }}]" value="5" name="skala[{{  $lkpSoalan->id_soalan }}]" required> Sangat Baik &emsp;
							<input type="radio" id="skala[{{  $lkpSoalan->id_soalan }}]" value="4"  name="skala[{{  $lkpSoalan->id_soalan }}]" required> Baik &emsp;
							<input type="radio" id="skala[{{  $lkpSoalan->id_soalan }}]" value="3"  name="skala[{{  $lkpSoalan->id_soalan }}]"  required> Memuaskan &emsp;
							<input type="radio" id="skala[{{  $lkpSoalan->id_soalan }}]" value="2"  name="skala[{{  $lkpSoalan->id_soalan }}]" required> Kurang Memuaskan &emsp;
							<input type="radio" id="skala[{{  $lkpSoalan->id_soalan }}]" value="1"  name="skala[{{  $lkpSoalan->id_soalan }}]"  required> Sangat Tidak Memuaskan &emsp;
						</div>
					</div>
					@endforeach
