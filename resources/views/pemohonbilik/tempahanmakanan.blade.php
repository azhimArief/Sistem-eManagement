...
@section('content')
    ...
    <div class="container" align="center">
        ...
        <div class="col-sm-11">
            <div class="card card-info text-lg-left"> 
                {{-- <label for=""><font color="red">&nbsp 1. Tekan <button type="button"  class="btn btn-blue btn-sm bg-blue"><i
                    class="fa fa-save"></i> | Simpan</button> untuk mengisi tempahan makanan.</font> 
                </label>
                
                <label for=""><font color="red">&nbsp 2. Tekan <button type="button"  class="btn btn-purple btn-sm bg-purple"><i
                    class="fa fa-paper-plane"></i> | Selesai</button> setelah selesai dengan tempahan makanan anda.</font> 
                </label> --}}
                
                <label for="">&nbsp 1. Sila masukkan maklumat yang diperlukan dan tekan butang hantar setelah selesai.</label>
                <label for="">&nbsp 2. <font color="red">Menu adalah tertakluk kepada kelayakan kadar pengerusi.</font></label>
                <label for="">&nbsp 3. Tempahan bilik hanya boleh diterima jika ditempah 1 hari sebelum tarikh tempahan.</label>
                <label for="">&nbsp 4. Tempahan makanan hanya boleh diterima jika ditempah 2 hari sebelum tarikh tempahan.</label>
                <label for="">&nbsp 5. Untuk melihat panduan menu hidangan <a href="{{ url('uploads/BUKU PANDUAN MENU DAN KALORI.pdf') }}" target="_blank">Klik Sini</a></label>
        ...
        <div class="col-sm-11">
            <div class="card card-info">
                ...
                <form role="form" action="{{ route('pemohon.simpan_makanan', $tempahmakan->id_tempah_makan) }}" method="POST"
                                    ...
                                    <div class="container">
                                        ...
                                        @foreach ($menumakan as $menumakans)
                                            ...
                                            <div class="modal fade" id="modal-makan-{{  $menumakans->id_menu_makan }}">
                                                <div class="modal-dialog">
                                                    <div class="modal-content">
                                                        ...
                                                        <form class="form-horizontal" method="POST" action="{{ route('pemohon.simpan_makanan', $tempahmakan->id_tempah_makan) }}" enctype="multipart/form-data">
                                                            {{ csrf_field() }}
                                            ...
                                            @if($menumakans->jenis_makan == '1')
                                                <tr>
                                                    ...
                                                    <td align="center">
                                                        <a href="{{ route('pemohon.hapus_makanan',$menumakans->id_menu_makan) }}" title="Buang" class="btn-sm btn-danger"> <i class="fa fa-trash"></i></a>
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-3">
                                                ...
                                                <select class="form-control select2" id="id_jenis_hidangan1" name="id_jenis_hidangan1" style="width: 100%;" required>
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-3">
                                                <div class="input-group">
                                                    ...
                                                    <textarea name="input_pagi" class="form-control" id="input_pagi" cols="30" rows="2" placeholder="Cth: nasi ayam">{{ old('input_pagi') }}</textarea>
                                                </div>
                                            </div>
                                        </div>
                                            ...
                                            <div class="col-sm-3">
                                                <div class="input-group">
                                                    <input type="text" class="form-control" id="kalori_pagi" name="kalori_pagi"
                                                        placeholder="Cth: 100" min="1" max="40"
                                            ...
                                            <div class="col-sm-3">
                                                <div class="input-group">
                                                    <input type="text" class="form-control" id="kalori_petang" name="kalori_petang"
                                                        placeholder="Contoh: 100" min="1" max="40"
                    ...
                    <div class="card-footer justify-content-center">
                            <button type="submit" name="hantar" class="btn btn-purple float-left btn-sm bg-purple"><i
                                    class="fa fa-paper-plane"></i> | Hantar</button>
                        {{-- <button type="button" class="btn btn-default float-right btn-sm" onclick="history.back();"><i
                                class="fas fa-redo-alt"></i> | Kembali</button> --}}
                            <a href="{{ url('/pemohon/tempahanbilik/kemaskini/' . $tempahmakan->id_permohonan_bilik) }}" class="btn btn-sm btn-default float-right" title="Kembali Ke Tempah Bilik"><i class="fas fa-redo-alt"></i> | Kembali</a>
                    </div>
...
@section('script')
    ...
    <script type="text/javascript">
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        for (var i = 0; i < data.length; i++) {
                            op += '<option value="' + data[i].name + '">' + data[i].name +
                                '</option>';
                        }
        ...
        function tambahPenumpang() {
            var maklumat_penumpang = btoa('x|x' + $('#bahagian_penumpang').select2('data')[0].id + 'x|x' + $(
                    '#bahagian_penumpang').select2('data')[0].text + 'x|x' + $('#nama_penumpang').select2('data')[0]
                .id + 'x|x' + $('#nama_penumpang').select2('data')[0].text + 'x|x' + $('#no_tel_penumpang').val() +
                'x|x' + $('#emel_penumpang').val() + 'x|x' + $('#mykad_penumpang').val()) + '|x|x|';
        ...
        function updatePenumpang() {
            ...
            for (var mj = 0; mj < splitMJ.length - 1; mj++) {
                ...
                var emel = mAhli[6];
                var mykad = mAhli[7];
                newSenMJ = newSenMJ + '<tr><td>' + bil + '</td><td>' + nama + '</td><td>' + bahagian + '</td><td>' +
                    no_tel + '</td><td>' + emel +
                    '</td><td><button type="button" class="btn btn-sm btn-danger" id="btnRemovePenumpang' + mj +
                    '" onclick="removePenumpang(' + mj + ');" title="Hapus"><i class="fa fa-trash"></i></button></td></tr>';
            }
            $('#senMJ').html(newSenMJ);
        }
