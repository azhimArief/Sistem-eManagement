...
<body>
    ...
    <justify>
    <p>2. &emsp;&emsp;Adalah dimaklumkan status tempahan Kenderaan Jabatan Kementerian Perpaduan Negara adalah seperti berikut:</p>

    <p> 
        <table width="650" border="0" style="border-collapse:collapse;  font-size:13px; text-align:left; margin:0px 0px 0px 40px;" cellpadding="3" >
            <tr>
			...
			<tr>
                ...
                <td><strong>: </strong>@foreach($penumpangs as $penumpang)
            ...
            @if($content->id_status==3)
                <?php 
                    if($content->kenderaan_pergi != null)
                    {
                        $pemandu_pergi = \App\Kenderaan::where('id_kenderaan',$content->kenderaan_pergi)->first();
                        $pemandu_go = true;
                    }
                    else{
                        $pemandu_go = false;
                    }
                    if($content->kenderaan_balik != null)
                    {
                        $pemandu_balik = \App\Kenderaan::where('id_kenderaan',$content->kenderaan_balik)->first();
                        $pemandu_return = true;
                    }
                    else{
                        $pemandu_return = false;
                    }
                    
                ?>
                <tr>
                    <th style="text-align:left">Pemandu</th>
                    @if($pemandu_go == true)
                        <td>:<strong>&nbsp; KPN&rarr;{{ $content->lokasi_tujuan }}, {{ ($content->id_negeri !='') ? App\RLkpNegeri::find($content->id_negeri)->negeri : 'Tiada Maklumat' }}</strong>
                            <br>&nbsp; {{  ($pemandu_pergi->pemandu != '') ? App\LkpPemandu::find($pemandu_pergi->pemandu)->nama_pemandu : 'Tiada Maklumat' }} (Tel.: {{  ($pemandu_pergi->pemandu != '') ? App\LkpPemandu::find($pemandu_pergi->pemandu)->no_tel_bimbit : 'Tiada Maklumat' }})
                        </td>
                    @endif
                </tr>
                @if($pemandu_return == true)
                <tr>
                    <th style="text-align:left; vertical-align: top;">&nbsp;</th>
                    <td>:<strong>&nbsp; {{ $content->lokasi_tujuan }}, {{ App\RLkpNegeri::find($content->id_negeri)->negeri }}&rarr;KPN</strong>
                        <br>&nbsp; {{  App\LkpPemandu::find($pemandu_balik->pemandu)->nama_pemandu }} (Tel.: {{  App\LkpPemandu::find($pemandu_balik->pemandu)->no_tel_bimbit }})
                    </td>
                </tr>
                @endif
            @endif
    ...
    <p>Status tempahan Kenderaan Jabatan juga boleh disemak melalui <a href="{{ url('') }}">Sistem eTempahan</a>.
