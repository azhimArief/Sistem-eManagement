    ...
    <p> 
        <table width="650" border="0" style="border-collapse:collapse;  font-size:13px; text-align:left; margin:0px 0px 0px 40px;" cellpadding="3" >
            ...
            <tr>
                ...
                <?php $pemandu_pergi=\App\Kenderaan::find($content->kenderaan_pergi)->pemandu;
                    if($content->kenderaan_balik) { $pemandu_balik=\App\Kenderaan::find($content->kenderaan_balik)->pemandu;} ?>
                
                <td>:<strong>&nbsp; KPN&rarr;{{ $content->lokasi_tujuan }}, {{ \App\RLkpNegeri::find($content->id_negeri)->negeri }}</strong>
                     <br>&nbsp; {{  \App\LkpPemandu::find($pemandu_pergi)->nama_pemandu }} (Tel.: {{  \App\LkpPemandu::find($pemandu_pergi)->no_tel_bimbit }})
                </td>
            </tr>
            @if($content->id_jenis_perjalanan!==1)
            <!-- perjalanan pergi sahaja -->
            <tr>
                ...
                <?php $pemandu_balik=\App\Kenderaan::find($content->kenderaan_balik)->pemandu; ?>
                <td>:<strong>&nbsp; {{ $content->lokasi_tujuan }}, {{ \App\RLkpNegeri::find($content->id_negeri)->negeri }}&rarr;KPN</strong>
                     <br>&nbsp; {{  \App\LkpPemandu::find($pemandu_balik)->nama_pemandu }} (Tel.: {{  \App\LkpPemandu::find($pemandu_balik)->no_tel_bimbit }})
                </td>
            </tr>
            @endif
            @endif
